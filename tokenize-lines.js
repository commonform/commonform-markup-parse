// This module exports the top-level tokenizer ("lexical analyzer" or
// "lexer") for Common Form markup. The exported function deals with
// tokenizing lines of input. It utilizes the tokenizer in
// tokenize-content.js to handle tokens within lines of input.
...
//
// Line and column numbers are 1-indexed. (1 is the first line and the
// first column in each line.) String values for certain tokens, like
// outdents, are empty. Token type values are defined in tokens.json.
function tokenize (text) {
  ...
  // Split into lines.
  .split(NEWLINE)
  // For each line, create Array of tokens for indentation and content.
  .map(function (line, index) {
    if (ALL_SPACE.test(line) || COMMENT.test(line)) {
      ...
      var contentTokens = tokenizeContent(
        content, lineNumber, contentColumn
      )
      var arrayOfTokens = []
  ...
  // Flatten array of arrays of tokens into one long array of tokens.
  .reduce(function (tokens, array) {
    return tokens.concat(array)
  }, [])
  // Add OUTDENT tokens for any INDENT tokens that haven't been
  // cancelled out by an OUTDENT token yet.
  .concat(
    times(
      {
        type: 'OUTDENT',
  ...
  // Add the terminal END token, sometimes called EOF or ENDMARKER.
  .concat({
    type: 'END',
    line: lastLine,
    column: lastColumn,
